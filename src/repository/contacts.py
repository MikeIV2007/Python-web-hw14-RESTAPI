...
async def get_contact_by_name(
    contact_name: str, limit: int, offset: int, current_user: User, db: Session
):
    """
...
async def get_contact_by_surname(
    contact_surname: str, limit: int, offset: int, current_user: User, db: Session
):
    """
    ...
    contacts = db.query(Contact).filter_by(
        surname=contact_surname, user_id=current_user.id
    )
    contacts = contacts.limit(limit).offset(offset).all()
    return contacts
...
async def get_contact_by_email(contact_email: str, current_user: User, db: Session):
    ...
    contact = (
        db.query(Contact)
        .filter_by(email=contact_email, user_id=current_user.id)
        .first()
    )
    return contact
...
async def get_contacts_with_birthdays_in_next_7_days(
    limit: int, offset: int, current_user: User, db: Session
):
    """
...
async def update_contact(
    body: ContactModel, contact_id: int, current_user: User, db: Session
):
    """
