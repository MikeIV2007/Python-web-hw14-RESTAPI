...
@router.get(
    ...
    name="Get all contacts mo more than 10 requests per minute",
...
async def get_contacts(
    ...
    :doc-author: Trelent
    """
    contacts = await repository_contacts.get_all_contacts(
...
@router.get(
    "/id/{contact_id}", response_model=ResponseContact, name="Find contact by ID"
)
...
async def get_contact_by_name(
    ...
    :doc-author: Trelent
    """
    contacts = await repository_contacts.get_contact_by_name(
        contact_name, limit, offset, current_user, db
    )
    if contacts is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contacts with name {contact_name} not found",
        )
    return contacts
...
@router.get(
    ...
    response_model=list[ResponseContact],
    name="Find contact by surname",
)
async def get_contact_by_surname(
    contact_surname: str,
    limit: int = Query(10, le=1000),
    offset: int = 0,
    db: Session = Depends(get_db),
    ...
    :doc-author: Trelent
    """
    contacts = await repository_contacts.get_contact_by_surname(
        contact_surname, limit, offset, current_user, db
    )
    if contacts is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contacts with surname {contact_surname} not found",
        )
    return contacts
...
async def get_contact_by_email(
    ...
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contact with email {contact_email} not found",
        )
    return contact
...
async def get_contacts_with_birthdays_in_next_7_days(
    ...
    :doc-author: Trelent
    """
    contacts = await repository_contacts.get_contacts_with_birthdays_in_next_7_days(
        limit, offset, current_user, db
    )
    if not contacts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Contacts with birthdays for the next week not found",
        )
    return contacts
...
@router.delete(
    ...
    name="Delete contact form database by ID",
)
