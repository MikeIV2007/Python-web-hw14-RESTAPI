...
from src.repository.contacts import (
    ...
    get_contacts_with_birthdays_in_next_7_days,
    create_contact,
    update_contact,
    remove_contact,
)
...
class TestContacts(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_contacts_with_birthdays_in_next_7_days(self):
        test_contacts = [Contact(), Contact(), Contact()]
        self.session.query().filter().limit().offset().all.return_value = test_contacts
        result = await get_contacts_with_birthdays_in_next_7_days(
